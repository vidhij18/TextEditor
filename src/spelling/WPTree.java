...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		Queue<WPTreeNode> toExplore = new LinkedList<>();
		ArrayList<String> visited=  new ArrayList<String>();
		List<WPTreeNode> forDebug = new ArrayList<>();
		
		//parent node set as root  node of the WPTreeNode
		...
		while(!curr.getWord().equals(word2) && !toExplore.isEmpty())
		{
			curr = toExplore.poll();
			...
			for(String n: neighbors){
				...
				if(!visited.contains(n)){
					...
					WPTreeNode  newnode = new WPTreeNode(n, curr);
					toExplore.add(newnode);
					forDebug.add(newnode);
				}
		...
		//System.out.println("Printing "+printQueue(forDebug));
